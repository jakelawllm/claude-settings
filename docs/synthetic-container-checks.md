...
This runbook describes the controlled engineering environment tested on 2026-09-08. The nested sandbox, managed tool paths and encrypted synthetic-record recovery now work on the reviewed host. The final signed adapter has created its restricted target and passed the full 49-check managed harness; signed round-trip and restart verification also passed. Interactive entry and `/status` were subsequently observed after a bounded first-run metadata repair and normal workspace-trust confirmation, then observed again after a controlled restart without repeated setup prompts. The [current delegated owner decision](policy-decisions/internal-mvp-owner-acceptance.md) adopts these results for the synthetic MVP and accepts its supervised records process. Confidential matters and client retention obligations remain outside that approval. See [architecture](production-architecture.md), [OS acceptance](os-isolation-acceptance.md) and [remaining issues](INTERNAL_MVP_REMAINING_ISSUES.md).
...
|---|---|
| `mvp-signed-managed-20260908` | Final adapter-created target. Staging passed 12 egress checks before model use. Its subsequent full managed harness passed 49 checks, with no sibling inode events during denial probes and synthetic environment canaries scrubbed. The signed lifecycle then passed all seven steps, including independently checked persistence across restart. Actual `/status` subsequently confirmed the managed source, client version and Smith context after normal workspace-trust confirmation. |
...
This demonstrates restoration with a retained key, not recovery after losing the source host and key. Supervised archival and original-key availability are now accepted for synthetic testing under the delegated owner decision. Independent custody, client retention/hold and production records-owner acceptance remain requirements before confidential use. The repository's convenience archive has not become an external records service through this drill.
...
## Verified interactive entry and first-run setup repair

The final signed target's `/status` was actually observed on 2026-09-08. It displayed Claude 2.1.263, managed-settings source and the selected Smith context. The normal workspace-trust dialog remained active: its default rejection was respected, and the known synthetic Smith directory was then explicitly trusted through that UI. Evidence: `$MVP_ROOT/acceptance/interactive-status-verified.json`. Account identifiers and raw terminal content were not retained in the summary.

Earlier entry attempts reached theme/subscription onboarding despite independently verified existing authentication. Their attempt-only results remain in `interactive-status-final.json` and the earlier trust-attempt records; they are not retroactively counted as successful `/status` observations. The earlier advice that owner OAuth repetition was mandatory is superseded by inspection of this installed client's source and the executed repair.

In 2.1.263, the first-run completion callback writes only `hasCompletedOnboarding=true` and `lastOnboardingVersion="2.1.263"`. Its Security notes are advisory text; their confirmation advances the onboarding screen without changing authentication, organization restrictions, permissions, hooks, sandbox enforcement or workspace trust. Separate policy-consent dialogs also remain outside that gate. After independently verifying the existing owner-authenticated session, the operator repaired exactly those two absent preference fields. Every other configuration field remained equal, and credential files were not opened or changed. Evidence: `$MVP_ROOT/acceptance/onboarding-metadata-repair.json`.

This was a bounded metadata repair for the already authenticated test target. It does not claim fresh OAuth or a click on the advisory Security notes. Authentication, managed policy, organization checks, normal trust and sandbox controls were preserved. A genuinely new or expired login still requires the normal authentication flow. Do not generalize this client-specific repair into an authentication or permissions bypass.

From a real terminal, connect to the acceptance host using the operator's existing SSH target:
...
The prepared target can now enter its normal session. Use `/status` to compare the installed version, managed-settings source and selected synthetic matter with the protected release configuration, then `/exit` to leave normally. Keep login codes and account details out of chat, shared logs and Git. If credentials are genuinely absent or expired, complete normal OAuth in the trusted terminal/browser; the metadata repair is not a replacement for authentication.

After the metadata repair, signed verification exited 0 and the signed Bash smoke produced its required successful tool result (`launcher/session-results/20260908T050050Z-4fc41e`). The controlled restart then exited 0 with confinement and all 12 egress denials passing. Actual interactive `/status` after that restart passed five assertions: the status/settings labels, managed source, client 2.1.263 and Smith context were observed. Setup and trust prompts did not repeat, and normal `/exit` returned 0. Evidence: `$MVP_ROOT/acceptance/interactive-status-restart-verified.json`. This recovery observation is separate from the earlier seven-step persistence sequence. The earlier v5 candidate remains stopped/disconnected, and the ordinary-bridge authentication harness is stopped with its private home volume preserved.
