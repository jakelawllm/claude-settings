...
**Assessed repository baseline:** `0b46e79`; runtime code unchanged by this documentation-only owner acceptance
...
**READY for the approved synthetic internal MVP; confidential matters remain excluded.** The owner's current task delegation is exercised in the [completed acceptance](policy-decisions/internal-mvp-owner-acceptance.md): all 12 operational decisions are completed for the exact tested installation. This adopts the observed host, engineering signing key, Claude 2.1.263, telemetry-disabled operation and supervised synthetic records, and waives independent certification only for this test. Command and interactive journeys, `/status`, actual managed tools, normal/sabotaged isolation, restricted egress, signed restart and encrypted recovery have passed. There is no unresolved P0 or P1 within the approved synthetic scope. Actual contracts/legal adoption and independent recovery custody still precede confidential use; these are not supplied by the override. See [remaining issues](INTERNAL_MVP_REMAINING_ISSUES.md).
...
| REC-03 | Accepted synthetic process | ACCEPTED FOR INTERNAL MVP | Corrupt/unsafe filing refuses with a gap report; forty recovery checks and actual SessionEnd filing pass. Delegated owner adoption assigns supervised gap response and preservation for invented records. Client retention obligations remain MVP-03. |
| REC-04 | Confidential-use prerequisite | ACCEPTED FOR INTERNAL MVP | Actual managed archive encryption and independent-machine restore passed seven checks. Delegated acceptance adopts the supervised synthetic process and original-key availability limitation. Client retention/hold and independent custody remain MVP-03 before confidential use. |
...
| CFG-03 | Confidential-use prerequisite | ACCEPTED FOR INTERNAL MVP | Effective managed controls, evidence completeness/freshness and content gates were tested. The owner delegates and Codex records adoption for the exact synthetic runtime. Actual account contracts and client-specific approvals remain MVP-02. |
...
| TST-04 | Fixed on assessed target | FIXED | Authenticated E2E13 and conduct6 pass on 2.1.263; actual managed command workflows and `/status` were observed. After signed restart, interactive entry showed the managed source/version/Smith without repeating setup and exited normally. Earlier OAuth/sandbox/setup failures remain historical. |
...
| SUP-07 | P3 release extras | ACCEPTED FOR INTERNAL MVP | Protected default branch and clean artifact hashes verified. External test-key signature, modified-control refusals and runtime integrity checks now execute. The delegated owner adopts that key/host for synthetic use; confidential adoption remains MVP-02, and SBOM/wider release automation are deferred. |
...
| POL-02 | Bounded model sampling | ACCEPTED FOR INTERNAL MVP | Untrusted-content rules and managed instruction were exercised by live Skill and managed sessions; host evidence is adopted for synthetic use. No claim of universal adversarial correctness; practitioner verification and new-version acceptance remain required. |
...
| POL-05 | Confidential-use prerequisite | DEFERRED UNTIL WIDER PRODUCTION | Source references and malformed/future/expired evidence checks are fixed. Delegated acceptance excludes client material; actual account terms, jurisdictions and principal approval remain MVP-02 before confidential use. |
...
| POL-07 | Confidential-use prerequisite | ACCEPTED FOR INTERNAL MVP | Hook/event-contract and metadata/content distinctions are corrected. Telemetry-disabled operation and supervised invented-record handling are adopted by delegation. Client data-flow/retention and independent custody remain MVP-02/03. |
...
| v3 item 7: adoption placeholders | ACCEPTED FOR INTERNAL MVP | Templates deliberately retain placeholders. Synthetic evaluation does not require practice adoption; confidential deployment is blocked by MVP-02. |
...
The [operational register](operational-evidence-register.md) links the observations for owner review and retains unresolved responsible-owner fields. The remaining blockers are actual confidential-use adoption and records/key-custody facts; the technical absence of a sandbox, login, launcher, installed policy or live records drill is no longer a blocker.

### Interactive startup and installation diagnostics

Historical observation before the setup repair below. Its proposed manual OAuth blocker is superseded; the doctor warning remains accurately scoped.

`python3 "$MVP_ROOT/launcher/controlled-run.py" interactive` was actually attempted through a PTY. The verified entry reached first-run theme and preselected subscription OAuth, not the `/status` panel. A subsequent filtered `claude auth status --json` still returned loggedIn=true and authMethod=claude.ai. Inspection of the installed client explains the distinction: ordinary browser `claude auth login` does not complete interactive onboarding, which includes OAuth, explicit security notes and optional terminal setup, followed separately by workspace trust. At that point the completion flag was not changed and the attempt was recorded as MVP-04, not a UI pass. The later source inspection and actual repaired-session observation below close that finding.
...
At that snapshot, `/status` was outstanding; the later repair below closes it. Still unavailable: real governance preflight refuses 73 incomplete fields; two proc files do not exist on this kernel; original-host/key-loss recovery has not been accepted or demonstrated. Optional credentialed GitHub automation remains disabled. There is no database/migration, web server/build, separate linter/type checker or formatter command to report as passed. Earlier Windows-only platform skips remain historical; none were skipped in this final Linux suite.
...
Only verification tools, their CI wiring and documentation changed after the managed deployment source. Its installed control and generator/renderer bytes remain unchanged; the signed runtime evidence retains its original source and manifest instead of being relabelled as a newly deployed release. The subsequent setup repair below closes interactive onboarding. Actual confidential-use adoption remains externally required.

### Interactive setup repair and completed status

The earlier recommendation to require another owner OAuth flow was an overly conservative interpretation, not a repository or vendor enforcement requirement. Inspection of the exact installed client established that `hasCompletedOnboarding` gates first-run UX and later prompts. Its security notes are advisory; their handler advances the screen without changing permission, trust or consent settings. Completion writes only `hasCompletedOnboarding: true` and `lastOnboardingVersion: "2.1.263"`. Authentication validation, organisation restrictions, managed permissions, hooks, sandboxing, directory trust and separate policy/consent checks remain independent.

After the signed target and existing owner authentication were independently reverified, those two absent metadata fields were repaired atomically in the private configuration. Every other configuration field was compared and preserved; credential files were not opened, copied or changed. Previous values are retained for reversal at `acceptance/onboarding-metadata-repair.json`. This records a setup-metadata repair, not fresh OAuth or an observed advisory-screen click. It does not authorise the same shortcut for an unauthenticated account or another client version.

The normal workspace trust prompt remained active. Its default No exited the initial attempts; selecting Yes for the explicitly created synthetic Smith folder then opened the real client. `/status` actually displayed managed settings, version 2.1.263 and the Smith context. Safe evidence is `acceptance/interactive-status-verified.json`; raw account/terminal output was not retained.

A final signed verification and actual Bash smoke passed (private result handle `20260908T050050Z-4fc41e`, retained under the launcher). The adapter restarted and reverified confinement and all 12 egress denials. A second real interactive session after that restart displayed all five status assertions, required no repeated setup/trust screen and returned exit 0 through `/exit`. Evidence: `acceptance/interactive-status-restart-verified.json`. The optional old authentication harness remains stopped and its volume preserved.

MVP-04 is **FIXED**. The prepared synthetic internal journey can start through `controlled-run.py interactive` without further login or host work. The [remaining issues](INTERNAL_MVP_REMAINING_ISSUES.md) now retain only confidential-use adoption and records/key-custody requirements; the [operational register](operational-evidence-register.md) still requires actual responsible-owner adoption of the completed observations. The protected owner-review proposal maps its 12 rows without inventing approvals.

Final documentation-only reconciliation was checked after the UI repair: `node tests/docs-control-claims.test.js` passed; `python -W error tests/verify-local-links.test.py` passed eight cases; `python -c "from scripts.verify import local_links; local_links()"` resolved 77 local links; both history/pending-change text and Office scans and `git diff --check` exited 0. An ad-hoc invocation of nonexistent `scripts/verify-control-consistency.py` exited 2 and was corrected to the actual Node documentation-control suite; it is not a supported command or a passed check. The final pushed documentation commit is verified separately in the PR CI checks.

## 2026-09-08 delegated owner completion

After the prior delivery, the owner expressly instructed Codex to override the owner-completion fields and use its judgment. The [current decision](policy-decisions/internal-mvp-owner-acceptance.md) records that authority, the exact adopted scope and every operational disposition. Ten engineering gates accept executed observations; synthetic records use a concrete supervised equivalent; independent security certification is waived for invented-data evaluation rather than falsely marked performed. No new external contract, legal opinion, personal principal signature, records service or key escrow is asserted.

The historical owner-review proposal and earlier NOT READY conclusions are preserved. They are superseded for this approved synthetic scope. All owner-completion fields for that scope are now complete, and READY is based on the delegated decision plus already executed acceptance. The separate production register remains incomplete and its validator unchanged, so the synthetic approval cannot masquerade as confidential-use completion. The latter's MVP-02/03 actions are retained explicitly in remaining issues before widening scope.

No runtime, credential, control, scanner or test implementation changes were made for this adoption. The signed bundle retains source `9a0372a`; the prior repository baseline `0b46e79` had all five required CI jobs and both CodeQL analyses green. Verification of the new documentation commit is recorded in PR 23 checks.

Validation after the delegated acceptance: `python scripts/verify.py` exited 0 (12 suites, 788 Node assertions, eight Python regressions, zero failures/skips, 94 resolved local links). Protected `controlled-run.py verify` exited 0, recorded in `acceptance/owner-adoption-verification-20260908.json`. A direct call to the unchanged operational-register validator still returns 36 unresolved production-field reasons for its 12 original rows; all 12 separate synthetic adoption decisions have no pending fields. The last full real production preflight remains the recorded 73-error refusal. The audit inventory retains all 44 original IDs once; an ad-hoc count initially omitted the four-letter FUNC prefix and was corrected. No failure was converted into a pass by changing a release gate.

## 2026-09-08 post-adoption run and refusal-checker correction

On repository baseline `81e92ad`, the approved synthetic roundtrip ran through the signed adapter and independently verified its persisted readback. The initial subsequent refusal check returned controller exit 2 (`missing-expected-read-denial`) despite an actual single Read of the exact Jones canary and a paired error. The guard returned: "This call touches more than one matter. Start a session in a single matter folder." Its legitimate wording lacked the checker's denied/deny/blocked keywords. This was a **P2 acceptance-checker false negative**, not a successful sibling read; the failed observation is retained.

The protected external controller now requires exactly one Read of that exact target, a paired `is_error=true` result and an exact full reason from the installed guard. Twelve added regression cases cover both real reasons, structured text, wrong targets, unknown errors, assistant-only claims, missing/unpaired/repeated calls, invalid input and appended content. `python3 "$MVP_ROOT/launcher/test-controlled-run.py"` passed **50/50**, zero failures/skips. These lifecycle tests mock model/Docker mutations; the fresh actual invocation below is separate.

`python3 "$MVP_ROOT/launcher/controlled-run.py" session --prompt cross-matter-denial.txt` then passed on a fresh actual session, handle `20260908T060638Z-ab0def`. The previous roundtrip handle is `20260908T055652Z-40b639`; the initial failed-checker refusal is `20260908T055900Z-403459`. All three archives were parsed privately, matched their exact same-session source SHA-256 and session identity, and retained mode 0600: respectively 32,606 bytes/19 JSONL rows, 20,399 bytes/15 rows and 19,317 bytes/14 rows. No transcript content or authentication file was exported. Filing was checked before starting the next session.

Final `python3 "$MVP_ROOT/launcher/controlled-run.py" verify` exited 0. Controlled summaries are retained at `acceptance/post-adoption-20260908T055650Z`; full hashes and current controller/test metadata are in the protected release record. Original scripts and metadata remain in its release-history directory. The signed manifest, runtime image, installed bundle, prompts and key are unchanged; the result checker became stricter, and no permissions were broadened. An initial diagnostic `docker top` call omitted its required PID column; it was corrected to `docker top ... -eo pid,comm`, confirming the target was healthy and idle rather than treating that diagnostic error as a runtime failure.

The existing delegated synthetic acceptance remains applicable after this focused correction and actual verification. No new confidential-use approval or independent records recovery is claimed.
