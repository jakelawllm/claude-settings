...
- [x] OAuth SUP-05 exception complete (every Option B operational field non-PENDING) **or** workflow migrated to short-lived identity. **Complete for internal beta only — Option B owner packet filled 2026-08-14; first rotation due immediately on 2026-08-14 because token has never been rotated.** Production go still blocked: production preflight rejects an internal-beta-only static-token status. See `docs/policy-decisions/oauth-token-management.md`.
