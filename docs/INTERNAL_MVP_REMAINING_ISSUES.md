...
**Assessed commit:** `a4bb7adc85aa`

**Assessment date:** 2026-09-08

Offline synthetic repository, container-boundary and records-recovery checks pass. The separate authenticated synthetic harness now passes 13 E2E checks, six conduct samples and installation diagnostics. The managed target remains blocked by its failing nested sandbox and missing managed/launcher/approved-egress acceptance. Confidential-matter requirements below remain separate; the historical internal-beta waivers do not approve client data. Completed engineering work is recorded in the [assessment](INTERNAL_MVP_READINESS_REPORT.md) and [container runbook](synthetic-container-checks.md), not retained as open work here.
...
|---|---|---|---|---|---|---|---|
| MVP-01 | P1 | Required nested sandbox and managed deployment acceptance unavailable | [Current checks](synthetic-container-checks.md): Claude 2.1.263 authentication, 13 live E2E checks, six conduct samples, doctor and 35 offline boundary checks pass. The managed candidate still fails `clone(CLONE_NEWNS + CLONE_NEWUSER)` with EPERM. No completed installed organisation-bound policy, external launcher/signature or approved-egress acceptance. | Docker namespace restrictions and Ubuntu AppArmor capability restrictions are host policy. No safe image-only repair was established; scoped privileged profile inventory/loading is unavailable. The authenticated bridge harness cannot substitute for the accepted managed boundary. | Have the host administrator inspect denial records and provide reviewed candidate-specific AppArmor/seccomp profiles preserving confinement, or use a supported alternate host. Follow the runbook sequence; require the bubblewrap probe to exit 0, then rerun all boundary controls. Install actual account-bound managed settings and observe `/status`; execute every launcher, signature, approved-egress and authenticated OS acceptance row. | Deployment operator / host administrator | Actual nested-sandbox probe passes; installed managed policy and every [OS acceptance row](os-isolation-acceptance.md) have target evidence. No unconfined/broad-privilege or failIfUnavailable bypass; offline network-none and separate bridge-harness results remain scoped. |
...
- [ ] On the accepted managed target, restart the client with retained synthetic state and verify actual live SessionEnd filing. Offline container restart/state, direct-hook recovery and temporary live-harness archive persistence already pass.
