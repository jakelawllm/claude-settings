...
>
> **Only the Linux container path is certified for hard matter isolation in this release, and WSL2 running the same container path with it.** Native Windows is not a supported platform for matter isolation: the sandbox does not run there, and the shipped settings set `failIfUnavailable` to `false`, so Claude Code starts anyway; on those machines nothing at the operating system level contains a Bash command, and matter separation rests on the hook alone, which is advisory. macOS is a different and open question: Claude's sandbox *does* run there, so the boundary primitive exists, but this release has not certified the macOS-with-Claude-sandbox-alone path for hard matter isolation, and a practice relying on it must complete and record the OS isolation acceptance procedure in `docs/production-architecture.md` before treating it as certified. Where the boundary needs to hold and the acceptance procedure has not been completed, use a Linux container, or Windows with Claude Code inside WSL2 using the same Linux container path. If you deploy to native Windows, record in Schedule 8 that isolation is advisory there.
>
> **The PreToolUse matcher is the wildcard "*".** A tool that is not listed in the guard's capability registry is refused in enforce mode (it is treated as unknown). The registry must be extended when new built-in, plugin or MCP tools are introduced, or those tools will be denied until they are classified. The guard does not attempt to parse or rewrite Bash command strings.
...
`forceLoginOrgUUID` is present in the template as `REPLACE-WITH-YOUR-FIRM-CLAUDE-ORG-UUID`. Replace it with the practice's real Anthropic organisation UUID. Clause 6.8 prohibits the use of a personal account for any work of the practice, even where the underlying product is the same as an approved tool, because the account determines the contractual terms, the retention period and whether inputs are used for training. `forceLoginMethod` does not reach that: it restricts the sign-in method, not the account, so a personal Claude.ai subscription satisfies it. `forceLoginOrgUUID` is the key that enforces clause 6.8. An invalid value blocks every login, so replace the placeholder with the real value before production deployment.
...
`sandbox` is enabled, with `allowUnsandboxedCommands: false`. `failIfUnavailable` ships as `false` (for observation-period deployments where the sandbox may not yet be available). In production it must be set to `true` so that Claude Code refuses to start rather than run unprotected. The operating-system sandbox is what contains Bash and its child processes. Enabling the sandbox is necessary but not sufficient for per-matter isolation: without a per-matter `sandbox.filesystem` policy (deny broad, allow only the current matter plus approved tooling paths, plus the managed-read-paths-only lock), Claude's sandbox permits reads from the whole machine. This repository ships `scripts/generate-matter-sandbox.py` to generate that policy from an approved matter definition; it does not create the container or verify the host sandbox. On native Windows there is no OS sandbox equivalent; the guard is advisory for Bash on that platform. Set `failIfUnavailable: true` and record which machines actually have the sandbox in the practice's deployment record.
...
That is narrower than "no session touches two matters", and the difference matters. The PreToolUse matcher is the wildcard "*". A tool absent from the guard's capability registry is refused in enforce mode rather than passed through. The guard does not parse Bash command strings, so a shell command can address another matter by absolute path; containment of Bash comes from the operating-system sandbox (when it is both enabled and configured with a per-matter policy), not from the hook. New built-in, plugin or MCP tools must be added to the registry before they can be used in enforce mode. Where the sandbox cannot run or is not configured with a matter-scoped filesystem policy, the guard is advisory for those routes. This repository ships `scripts/generate-matter-sandbox.py` to generate a per-matter sandbox policy; until the rendered settings include such a policy and the host sandbox is verified, no part of the shipped template should be read as providing hard per-matter Bash isolation.
...
Two limits, stated rather than buried. **Bash is confined by working directory only** (the hook does not inspect or rewrite command strings), so a shell command can still read across matters. The Claude sandbox boundary primitive is available on macOS, Linux and WSL2 and not on native Windows; on native Windows there is no equivalent OS sandbox, so the guard is advisory for Bash and the matter boundary rests on the hook alone. Only the Linux container path and WSL2 running it are *certified* for hard matter isolation in this release; macOS with Claude sandbox alone is an open question and is not certified in this release. The sandbox, where it is used, must be enabled with `sandbox.failIfUnavailable: true` plus a per-matter `sandbox.filesystem` policy, and on macOS it must be taken through the OS isolation acceptance procedure in `docs/production-architecture.md` before being treated as certified. Where the boundary needs to hold, use the certified path. Hooks are client-side: they constrain the model, not a determined user or a process running outside the sandbox. This repository ships `scripts/generate-matter-sandbox.py` to generate a per-matter sandbox policy, but the deploying practice must still render it, install it and verify the host sandbox before relying on it.
...
The platform difference matters and is not cosmetic. The operating-system sandbox boundary primitive (the only boundary the OS enforces rather than the client) runs on macOS, Linux and WSL2 and not on native Windows. **Only the Linux container path and WSL2 running the same container path are *certified* for hard matter isolation in this release.** macOS with Claude sandbox alone is an open question and is not certified in this release; a practice that treats the macOS path as certified for confidential-matter use must complete and record the OS isolation acceptance procedure in `docs/production-architecture.md` before doing so. On native Windows the matter boundary is the hook alone, and the hook confines Bash by working directory only; a shell command can still read across matters. A practice running on Windows should know that it holds the weaker of the two positions, and that running Claude Code inside WSL2 obtains the stronger one. Native Windows is not supported for confidential-matter use where a hard boundary is required.
